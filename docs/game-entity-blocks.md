...
Pattern: `GameEntityBlock` had no named member at all, so
`<Verb>Entity<Noun>Block` came from the 30 named `IScriptBlock` siblings,
which fix `<Verb><Noun>Block` and attest the verbs `Get`, `Set`, `Create`,
`Destroy`, `Insert`, `Remove`, `Copy`, `Shuffle`, `Sort`, `Concatenate`,
`Preload`, `ForEach`. The suffix is optional - 7 of the 15 named
`LevelScriptBlock` children spell no `Block` - and four names below drop it
too. A block's fields are its signature: `Pointer I<T>Get` inputs,
`Embed <T>TableSet` / `<T>ArrayTableSet` outputs.

## Runs

| run | probes | states | expected noise | yield |
|---|---|---|---|---|
| slot proposals read off each member's input/output shape | 44 | ~2,400 | 2.5e-5 | 9, incl. the Set/Get transform accessors and the Spawn/Create pair |
| suffix folding, 11 suffixes over 41 targets | 451 | ~13k | 0.001 | 3 stems, below |
| structure-led recombination, verb x noun, 3 arrangements | 35,250 | 41 | 0.0003 | 18 |
| widened to prepositions, `<Verb><Noun>To/From<Noun>Block` | - | - | 0.004 | 3 |
| CommunityDragon strings (4,046,399 forms) | - | 261 | 0.246 | 0, a clean negative |
| Windows client strings (144,107 forms) | - | - | 0.009 | 2 field names, whole strings |
| macOS client strings (190,735 forms) | - | - | 0.012 | the same 2 |
| field recombination, `force` depth 2 full wordlist over 11 field hashes | 20,040,780 | - | 0.051 | `ShowDuration`, `HideDuration` |
| lattice completion, `MapIndex` x block templates | 33,696 | - | 0.0003 | 3 |
| lattice completion, `Group` for `Tag` swap | 386,130 | - | 0.004 | 2 |
| type closure, `<word><tail>` over the unresolved class set | 101,696 | - | 0.054 | 10, each landing on a class whose `I<T>Get` base is named for the same `T` |
| type closure, second sweep | 82,498 | - | 0.043 | 2 |

The folds, each one opening a lattice: `unfold(0x32790cbf, "EntityBlock")` =
`Destroy` and `unfold(0x2bfff098, "EntityBlock")` = `Show`, both known words,
so the `<Verb>EntityBlock` shape is fixed by the hashes, not proposed;
`0x5d85166e` and `0x7dcd3672` unfold to one shared stem under `Get` /
`TableGet`, and that stem is `0x44ee0c82`, an unresolved *field* hash the
family declares - the field names the type, `EntityTag`; `0x4dc8b7cf` and
`0xacf818f1` share a stem the same way, equal to `fnv("EntityGroup")`.

The client strings, both whole:
...
`ShowDuration` and `HideDuration` are the two floats that separate
`ActivateEntity*` from `DeactivateEntity*`, so the fields retro-confirm the
...
The transform accessors, where the Set half takes the value as an input and
the Get half writes it to a variable:
...
The script value type system the blocks are wired with. A filled cell is a
name this batch added, an *italic* cell was already named:
...
objects, in `LolSpellScript` and `BuffScript` on Global, Map11 and Map453.
Only three members ship at all, and all three carry their name:
...
`ExecuteFunctionEntityBlock` gets a second, independent check: the type
closure named `0x71cab931` `ExecuteFunctionBlock` from the `IRunFunctionBlock`
row, so the `ExecuteFunction` stem is attested on a class this campaign did
not aim at.
...
interfaces under `IScriptBlock` that declares nothing anywhere. The base and
the interface flag fit `I<Domain>Block`, and both fell out of one 0.054-noise
run, but there is no second slot to check them against and no shipped
instance. Anyone submitting this upstream should split those two off or say
so. `PlaceEntityBlock` is the same shape one tier better: it has a predicted
slot, but nothing beyond it.

**Casing.** `Deactivate` over `DeActivate`: the hash cannot tell them apart,
and upstream ships one of each (`DeactivateEarlySeconds`, `DeActivate`). The
one-word spelling is standard English and keeps `De` out of the wordlist,
where it would be a junk token. `MapIndex` and `EntityTemplateVar` are the
strings as shipped, and our tables already carry `EventMapIndex` and
`ShopMapIndex`.
...
None of the four has a shipped instance, so there is no graph to read. Ruled
out for them, so nobody re-treads: `identity` and `delete` under 8 prefixes
and 6 suffixes, `chain` depth 4, `force` depth 2 over the full wordlist under
6 suffixes, `force` depth 2 under 8 prefixes crossed with 7 suffixes, `force`
...
each. Summed expected noise about 3, and the only hits were the four
incoherent ones the two noisiest runs predicted.

The pair constraint was tried too and is worth recording as a negative:
whatever `94018232` is on the Activate blocks, `9e8fcd0c` is its Deactivate
counterpart, so a name of the form `Show<W>` / `Hide<W>` would have to satisfy
both hashes at once. No `W` in the wordlist does at depth 1 or 2, while the
control `ShowDuration` / `HideDuration` is found immediately. The two bools do
not share a tail under any verb pair the corpus knows.
...
lifespans meeting at one patch boundary rather than by any single hash; the
four lattice rows, proved by filling every cell at once; the two fields
carrying a client string; and `GetEntityPositionBlock`, which 39 shipped
instances name outright. The rest carry a slot argument, which is the standard
